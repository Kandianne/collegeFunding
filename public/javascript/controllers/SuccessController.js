...
(function() {
	...
	function SuccessController() {
		var vm = this;
			...
			vm.changeIntoArrOfObjects = function(){
				for(var i = 0; i < vm.colleges.length; i ++){
					myObjs = {colleges: vm.colleges[i], offers: vm.offers[i], projections: vm.projections[i]};
					vm.successes.push(myObjs);
				}
			}

			vm.changeIntoArrOfObjects();
			vm.awards = vm.successes.length - 1;

			console.log(vm.awards);
